...
public class AdministratorSceneController implements Initializable {
  ...
  public void updateUserFullName(String newText) {
    userFullName.setText(newText);
  }
  ...
  public void onAddNewProductButtonClick(ActionEvent event) {
    try {
      ...
      productSceneController.setIsEdit(false);
      ...
      stage.setTitle("ООО \"Товары для животных\" | Добавление товара");
      stage.show();
    } catch (IOException e) {
      System.err.println(e);
      e.printStackTrace();
  ...
  public void onUpdateProductsListButtonClick(ActionEvent event) {
    ...
    while (it.hasNext()) {
      Object row[] = (Object[]) it.next();
      System.out.printf("%s %s %s %s %s %s %s %s %s %s %s%n", row[0], row[1], row[2], row[3], row[4], row[5], row[6],
    ...
    session.close();
    SortByAscendingCostRadioButton.setSelected(true);
    manufacturerComboBox.setPromptText("Производитель");
    orderProductsVBoxByAscending();
    allElementsCountLabel.setText(Integer.toString(productsVBox.getChildren().size()));
    currentElementsCountLabel.setText(Integer.toString(productsVBox.getChildren().size()));
  }
